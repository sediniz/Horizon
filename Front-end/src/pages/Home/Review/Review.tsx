...
const Review: React.FC = () => {
  ...
  useEffect(() => {
    const updateReviewsPerPage = () => {
      setReviewsPerPage(window.innerWidth >= 1024 ? 2 : 1);
    };
  ...
  const prevReview = () => {
    ...
    setIsTransitioning(true);
    setTimeout(() => {
      setCurrentReview((prev) => (prev - reviewsPerPage + reviews.length) % reviews.length);
      setIsTransitioning(false);
    }, 150);
  };

  // Handle direct navigation from indicators
  const goToReview = (index: number) => {
    if (isTransitioning) return;
    setIsTransitioning(true);
    setTimeout(() => {
      setCurrentReview(index * reviewsPerPage);
      setIsTransitioning(false);
    }, 150);
  };
  ...
  const getCurrentReviews = () => {
    ...
    for (let i = 0; i < reviewsPerPage; i++) {
      ...
      if (reviews[index]) {
        currentReviews.push(reviews[index]);
      }
    }
  ...
  return (
    <div className="Review max-w-7xl mx-auto p-4 bg-white rounded-lg shadow-lg relative">
      {/* Botões de navegação */}
      <button
        onClick={prevReview}
      ...
      {/* Reviews Grid - 2 reviews on large screens, 1 on smaller screens */}
      <div className={`grid grid-cols-1 lg:grid-cols-2 gap-6 transition-all duration-300 ease-in-out ${
        ...
        {currentReviews.map((review, reviewIndex) => (
          <div 
            ...
            style={{
              transitionDelay: `${reviewIndex * 100}ms`
            }}
          >
            <div className="flex flex-col gap-4">
              {/* Lado superior: Conteúdo da review */}
              <div className="space-y-3">
                {/* Perfil - Simplified without avatar */}
                ...
                {/* Texto da avaliação - Without title */}
                <div className="space-y-2">
                  <p className="font-semibold text-gray-800 leading-relaxed text-md">
                    {review.comment}
                  </p>
              ...
              {/* Lado inferior: Imagem e informações do hotel */}
              <div className="space-y-3">
                {/* Imagem do hotel */}
                ...
                <div className="bg-gray-50 rounded-lg p-3 space-y-2">
                  {/* Nome e estrelas */}
                  <div>
                    <h5 className="font-bold text-base text-gray-800">{review.hotel.name}</h5>
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <span className="text-xs text-gray-600">{review.hotel.stars} estrelas</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 text-gray-600">
                    ...
                    <span className="text-xs">{review.hotel.location}</span>
                  </div>
                  ...
                  {/* Comodidades - Limited to show only first 3 */}
                  <div>
                    <div className="flex flex-wrap gap-1">
                      {review.hotel.amenities.slice(0, 3).map((amenity: string, index: number) => (
                        <span
                          key={index}
                      ...
                      {review.hotel.amenities.length > 3 && (
                        <span className="text-xs text-gray-500">
                          +{review.hotel.amenities.length - 3} mais
                        </span>
                      )}
                    </div>
                  </div>
